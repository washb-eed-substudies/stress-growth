...
#-------------------------------------
# 
fit <- gam(y~s(x,bs="cr") +s(w,bs="cr"), family=gaussian,data=df)
summary(fit)
...
df<-data.frame(x=pd$x, point.diff, lb.diff, ub.diff)
ggplot(df) + #geom_ribbon(aes(x=x, ymin=lb.diff, ymax=ub.diff)) + 
        geom_point(aes(x=x, y=lb.diff))+
        geom_point(aes(x=x, y=ub.diff), color="red")+
        geom_point(aes(x=x, y=point.diff), color="blue")
...
ytics <- c(-1,0,1,2, 4)
plot(smooth.diff$x,(smooth.diff$y),
     type="n",bty="n",las=1,
...
# add CIs (dashed)
# lines(plotx,exp(smooth.lb.diff),lty=2)
# lines(plotx,exp(smooth.ub.diff),lty=2)

# add CIs (shaded)
polygon(x=c(smooth.lb.diff$x,rev(smooth.ub.diff$x)),
        y=(c(smooth.lb.diff$y,rev(smooth.ub.diff$y))),
        col=cicol,border=NA)

# add the predicted values
lines(smooth.diff$x,exp(smooth.diff$y),lwd=2,col=linecol)

# add the rug
segments(x0=smooth.diff$x,y0=min(ytics)-0.1,y1=min(ytics)-0.05,col=rugcol)