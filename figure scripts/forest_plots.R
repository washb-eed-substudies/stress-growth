...
H1adj %>% filter(X=="t2_iso_pca", grepl("_t2", Y) )
...
plotdf$X_alt <- factor(plotdf$X,
                   ...
                   labels = c("Glucocorticoid Receptor Methylation (%)", 
                              "Methylation at NGFI-A transcription\nfactor binding site (%)",
...
plotdf %>% filter(Y=="LAZ -Year 1") %>% select(X, point.diff, lb.diff,ub.diff)
...
p2 <- ggplot(plotdf %>% filter(Y=="LAZ -Year 2"), 
  ...
  coord_flip() +
  labs(y = "Year-2 LAZ adjusted mean difference", x = "Child stress measure") +
...
ylimits = c(-0.51, 0.22)
p1 <- ggplot(plotdf %>% filter(Y!="LAZ -Year 2"), 
             (aes(x=X_alt, y=point.diff, color=Y))) + 
  geom_point(size=3, position = position_dodge(.5)) +
  geom_errorbar(aes(ymin=lb.diff, ymax=ub.diff),
                width = 0.75, size = 1, position = position_dodge(.5)) +
  ylim(ylimits) +
  ...
  scale_color_manual(values = tableau10[1:3]) +
  coord_flip() +
  labs(y = "Adjusted mean difference", x = "Stress exposure") +
  ggtitle("") + theme_ki() + 
  theme(legend.position = c(0.1,0.35), axis.text.y=element_text(hjust=0.95,vjust=0.5)) +
  guides(color = guide_legend("Growth Outcome", nrow = 3)) 
p1
...
ylimits2 = c(-0.2, 0.25)
p2 <- ggplot(plotdf %>% filter(Y=="LAZ -Year 2"), 
             (aes(x=X_alt, y=point.diff, color=group))) + 
  geom_point(size=3) +
  geom_errorbar(aes(ymin=lb.diff, ymax=ub.diff),
                width = 0.75, size = 1) +
  ylim(ylimits2) +
  ...
  #facet_wrap(~Y, ncol=1, scales="free") +
  coord_flip() +
  labs(y = "Year-2 LAZ adjusted mean difference", x = "Stress exposure") +
  ggtitle("") + theme_ki() + 
  theme(legend.position = c(0.175,0.8), axis.text.y=element_text(hjust=0.95,vjust=0.5)) +
  guides(color = guide_legend("Stress axis", nrow = 3)) 

p2

p <- plot_grid(p1, p2, labels = c("A", "B"), ncol = 1, rel_heights = c(1, 2))
ggsave(p, file=paste0(here::here(),"/figures/forest_plot_poster.jpeg"), height=7, width=10)
ggsave(p, file=paste0(here::here(),"/figures/forest_plot_poster.png"), height=7, width=10, dpi=600)
