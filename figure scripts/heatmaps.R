...
plot_sig_heatmap <- function (d, pval_var = "Pval", title = "", Outcome = "Growth Outcome", 
  ...
  hm <- ggplot(d, aes(x = Y, y = X_f, fill = pval_cat)) + 
    ...
    theme(#aspect.ratio = 1, 
          ...
          axis.text.x = element_text(size = 8, colour = textcol, angle=45, hjust=1, vjust=1), 
...
p_unadj_t2 <- plot_sig_heatmap(unadj_res %>% filter(group=="Oxidative stress (Year 1)"))
p_unadj_t3 <- plot_sig_heatmap(unadj_res %>% filter(group!="Oxidative stress (Year 1)"))

p_sens_t2 <- plot_sig_heatmap(sens_res %>% filter(group=="Oxidative stress (Year 1)"))
p_sens_t3 <- plot_sig_heatmap(sens_res %>% filter(group!="Oxidative stress (Year 1)"))


ggsave(p_main, file=paste0(here::here(),"/figures/heatmap_main.jpeg"), height=8, width=8)
ggsave(p_main_t2, file=paste0(here::here(),"/figures/heatmap_main_t2.jpeg"), height=8, width=8)
ggsave(p_main_t3, file=paste0(here::here(),"/figures/heatmap_main_t3.jpeg"), height=8, width=8)

ggsave(p_unadj_t2, file=paste0(here::here(),"/figures/heatmap_unadj_t2.jpeg"), height=8, width=8)
ggsave(p_unadj_t3, file=paste0(here::here(),"/figures/heatmap_unadj_t3.jpeg"), height=8, width=8)

ggsave(p_sens_t2, file=paste0(here::here(),"/figures/heatmap_sens_t2.jpeg"), height=8, width=8)
ggsave(p_sens_t3, file=paste0(here::here(),"/figures/heatmap_sens_t3.jpeg"), height=8, width=8)

p_glm <- plot_sig_heatmap(glm_res)
ggsave(p_glm, file=paste0(here::here(),"/figures/heatmap_glm.jpeg"), height=8, width=8)

#-------------------------------------------------------------------------------
#Growth exposure sensitivity
#-------------------------------------------------------------------------------
temp=growth_res
growth_res=temp

growth_res$Y_temp=growth_res$Y 
growth_res$Y=growth_res$X 
growth_res$X=growth_res$Y_temp
growth_res=clean_res(growth_res)
# growth_res$X=growth_res$Y
# growth_res$Y=growth_res$X_f




p_growth_t2 <- plot_sig_heatmap(growth_res %>% filter(group=="Oxidative stress (Year 1)"))
p_growth_t3 <- plot_sig_heatmap(growth_res %>% filter(group!="Oxidative stress (Year 1)"))

p_growth_t2 <- p_growth_t2 + xlab("Prior growth exposure") + ylab("Stress Outcome")
p_growth_t3 <- p_growth_t3 + xlab("Prior growth exposure") + ylab("Stress Outcome")


ggsave(p_growth_t2, file=paste0(here::here(),"/figures/heatmap_growth_t2.jpeg"), height=8, width=8)
ggsave(p_growth_t3, file=paste0(here::here(),"/figures/heatmap_growth_t3.jpeg"), height=8, width=8)

save(list=ls(pattern="p_"), file=paste0(here::here(),"/figures/supplementary/heatmaps.Rdata"))
# ggsave(hm1, file=paste0(here::here(),"/figures/main/heatmap1.jpeg"), height=8, width=8)
